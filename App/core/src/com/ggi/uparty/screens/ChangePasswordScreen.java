...
public class ChangePasswordScreen implements Screen, InputProcessor {
	...
	public boolean keyTyped(char character) {
		...
		else if (stage.getKeyboardFocus().equals(npass)) {
		...
		else if (stage.getKeyboardFocus().equals(cnpass)) {
			if (character == '' && cn.length() > 0) {
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		} else if (Intersector.overlaps(touch, npassB)) {
			stage.setKeyboardFocus(npass);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, cnpassB)) {
			stage.setKeyboardFocus(cnpass);
			Gdx.input.setOnscreenKeyboardVisible(true);
		}
