...
public class SettingsScreen implements Screen, InputProcessor {
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		if (Intersector.overlaps(touch, backB)) {
			...
			invite.toggle();
		} else if (Intersector.overlaps(touch, reviewB)) {
			review.toggle();
		} else if (Intersector.overlaps(touch, logoutB)) {
			logout.toggle();
		}
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if (Intersector.overlaps(touch, backB)) {
			...
			u.nextScreen = (new ChangeUserScreen(u));
		} else if (Intersector.overlaps(touch, changePB)) {
			u.nextScreen = ((new ChangePasswordScreen(u)));
		} else if (Intersector.overlaps(touch, inviteB)) {
			u.nextScreen = (new InviteNewScreen(u));
		} else if (Intersector.overlaps(touch, reviewB)) {
	...
	private void toggleOff() {
		...
		if (changeU.isChecked()) {
			changeU.toggle();
		}
		if (changeP.isChecked()) {
			changeP.toggle();
		}
		if (invite.isChecked()) {
			invite.toggle();
