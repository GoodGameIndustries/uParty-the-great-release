...
public class LoginScreen implements Screen, InputProcessor {
	...
	public void render(float delta) {
		...
		pic.setColor(1, 1, 1,
				((u.nextScreen != null && u.nextScreen instanceof HomeScreen) || u.nextScreen == null ? 1f : fade));
		...
		pic.draw(
				(remember.isChecked() ? u.assets.get("UI/CheckBoxChecked.png", Texture.class)
						: u.assets.get("UI/CheckBox.png", Texture.class)),
				rememberB.x, rememberB.y, rememberB.width, rememberB.height);
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if (Intersector.overlaps(touch, backB)) {
			...
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, rememberB)) {
			remember.toggle();
		}
