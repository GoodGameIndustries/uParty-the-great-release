...
public class CreateEventScreen implements Screen, InputProcessor {
	...
	public void render(float delta) {
		...
		pic.draw(stage.getKeyboardFocus() != null && stage.getKeyboardFocus().equals(description) ? tAC : tA,
				descriptionB.x - .025f * descriptionB.width, descriptionB.y - .1f * descriptionB.height,
				1.05f * descriptionB.width, 1.2f * descriptionB.height);
		pic.draw(stage.getKeyboardFocus() != null && stage.getKeyboardFocus().equals(location) ? tAC : tA,
				locationB.x - .025f * locationB.width, locationB.y - .1f * locationB.height, 1.05f * locationB.width,
				1.2f * locationB.height);
	...
	public boolean keyTyped(char character) {
		...
		else if (stage.getKeyboardFocus().equals(name)) {
			if (character == '' && n.length() > 0) {
		...
		else if (stage.getKeyboardFocus().equals(description)) {
			if (character == '' && d.length() > 0) {
		...
		else if (stage.getKeyboardFocus().equals(location)) {
			if (character == '' && l.length() > 0) {
		...
		// start date
		else if (stage.getKeyboardFocus().equals(start.half)) {
			start.isPM = !start.isPM;
			stage.setKeyboardFocus(null);
		}

		if (stage.getKeyboardFocus().equals(start.minute)) {
			if (character == '' && start.min.length() > 0) {
		...
		else if (stage.getKeyboardFocus().equals(start.hour)) {
			if (character == '' && start.hr.length() > 0) {
				start.hr = start.hr.substring(0, start.hr.length() - 1);
			} else if ((character == '\r' || character == '\n' || !Character.isDigit(character))) {
			} else {
				start.hr += character;
				if (start.hr.length() == 2) {
					start.min = "";
					stage.setKeyboardFocus(start.minute);
				}
			}
		}

		else if (stage.getKeyboardFocus().equals(start.year)) {
			if (character == '' && start.y.length() > 0) {
				...
				if (start.y.length() == 4) {
					start.hr = "";
					stage.setKeyboardFocus(start.hour);
				}
			}
		}

		else if (stage.getKeyboardFocus().equals(start.day)) {
			if (character == '' && start.d.length() > 0) {
				...
				if (start.d.length() == 2) {
					start.y = "";
					stage.setKeyboardFocus(start.year);
				}
			}
		}

		else if (stage.getKeyboardFocus().equals(start.month)) {
			if (character == '' && start.m.length() > 0) {
				...
				if (start.m.length() == 2) {
					start.d = "";
					stage.setKeyboardFocus(start.day);
				}
			}
		}

		// end date
		else if (stage.getKeyboardFocus().equals(end.half)) {
			end.isPM = !end.isPM;
			stage.setKeyboardFocus(null);
		}

		else if (stage.getKeyboardFocus().equals(end.minute)) {
			if (character == '' && end.min.length() > 0) {
		...
		else if (stage.getKeyboardFocus().equals(end.hour)) {
			if (character == '' && end.hr.length() > 0) {
				...
				if (end.hr.length() == 2) {
					...
					stage.setKeyboardFocus(end.minute);
				}
			}
		}

		else if (stage.getKeyboardFocus().equals(end.year)) {
			if (character == '' && end.y.length() > 0) {
				...
				if (end.y.length() == 4) {
					...
					stage.setKeyboardFocus(end.hour);
				}
			}
		}

		else if (stage.getKeyboardFocus().equals(end.day)) {
			if (character == '' && end.d.length() > 0) {
				...
				if (end.d.length() == 2) {
					...
					stage.setKeyboardFocus(end.year);
				}
			}
		}

		else if (stage.getKeyboardFocus().equals(end.month)) {
			if (character == '' && end.m.length() > 0) {
				...
				if (end.m.length() == 2) {
					...
					stage.setKeyboardFocus(end.day);
				}
			}
		}
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if (Intersector.overlaps(touch, backB)) {
			...
			if (n.length() > 0 && d.length() > 0 && l.length() > 0 && start.isFilled() && end.isFilled()) {
				...
				Event event = new Event(u.controller.getLong(), u.controller.getLat(), n, d, l, start, end, u.myAcc.e,
						u.myAcc.xp);
				if (g != null) {
		...
		} else if (Intersector.overlaps(touch, nameB)) {
			stage.setKeyboardFocus(name);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, descriptionB)) {
			stage.setKeyboardFocus(description);
			Gdx.input.setOnscreenKeyboardVisible(true);
			scr = .07f * u.h;
		} else if (Intersector.overlaps(touch, locationB)) {
		...
		/** start date */
		else if (Intersector.overlaps(touch, start.monthB)) {
			start.m = "";
			stage.setKeyboardFocus(start.month);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, start.dayB)) {
			start.d = "";
			stage.setKeyboardFocus(start.day);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, start.yearB)) {
			start.y = "";
			stage.setKeyboardFocus(start.year);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, start.hourB)) {
			start.hr = "";
			stage.setKeyboardFocus(start.hour);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, start.minuteB)) {
			start.min = "";
			stage.setKeyboardFocus(start.minute);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, start.halfB)) {
			start.isPM = !start.isPM;
		}

		/** end date */
		else if (Intersector.overlaps(touch, end.monthB)) {
			end.m = "";
			stage.setKeyboardFocus(end.month);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, end.dayB)) {
			end.d = "";
			stage.setKeyboardFocus(end.day);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, end.yearB)) {
			end.y = "";
			stage.setKeyboardFocus(end.year);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, end.hourB)) {
			end.hr = "";
			stage.setKeyboardFocus(end.hour);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, end.minuteB)) {
			end.min = "";
			stage.setKeyboardFocus(end.minute);
			Gdx.input.setOnscreenKeyboardVisible(true);
		} else if (Intersector.overlaps(touch, end.halfB)) {
			end.isPM = !end.isPM;
		}
