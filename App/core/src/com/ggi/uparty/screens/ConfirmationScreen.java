...
public class ConfirmationScreen implements Screen, InputProcessor {
	...
	public boolean keyTyped(char character) {
		if (stage.getKeyboardFocus().equals(codeField)) {
			if (character == '' && codeInput.length() > 0) {
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		if (Intersector.overlaps(touch, resendB)) {
			resend.toggle();
		} else if (Intersector.overlaps(touch, contB)) {
			cont.toggle();
		}
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if (Intersector.overlaps(touch, resendB)) {
			...
			r.e = u.myAcc.e;
			u.send(r);
			u.error = "Code has been resent";
		} else if (Intersector.overlaps(touch, contB)) {
			if (("" + u.myAcc.code).equals(codeInput)) {
				Confirm c = new Confirm();
				c.e = u.myAcc.e;
				u.myAcc.confirmed = true;
				u.send(c);
				u.nextScreen = new MainScreen(u);
			} else {
				u.error = "Incorrect code";
			}
		} else if (Intersector.overlaps(touch, codeFieldB)) {
