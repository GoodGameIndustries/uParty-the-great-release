...
public class CommentScreen implements Screen, InputProcessor{
	...
	public void render(float delta) {
		...
		pic.draw(stage.getKeyboardFocus()!=null&&stage.getKeyboardFocus().equals(comment)?tAC:tA,commentB.x-.025f*commentB.width,commentB.y-.1f*commentB.height,1.05f*commentB.width,1.2f*commentB.height);
	...
	public boolean keyTyped(char character) {
		if(stage.getKeyboardFocus().equals(comment)){if(character == ''&&g.length()>0){
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		if(Intersector.overlaps(touch, backB)){goB = true;}
			...
			c.c=u.myAcc.xp+":"+g;
			c.e=u.myAcc.e;
			c.group=group==null?"":group.name.replace(" ", "")+group.owner.replace(".", "_").replace("@", "_");
		...
		else if(Intersector.overlaps(touch, commentB)){stage.setKeyboardFocus(comment);Gdx.input.setOnscreenKeyboardVisible(true);}
