...
public class ConfirmDeleteScreen implements Screen,InputProcessor {
	...
	public void render(float delta) {
		...
		u.mediumFnt.draw(pic, "Are you sure you want to delete this group?", u.w/2-layout.width/2, .55f*u.h-layout.height/2);
	...
	public boolean touchDown(int screenX, int screenY, int pointer, int button) {
		...
		if(Intersector.overlaps(touch, yesB)){yes.toggle();}
		else if(Intersector.overlaps(touch, noB)){no.toggle();}
		return true;
	}
	...
	public boolean touchUp(int screenX, int screenY, int pointer, int button) {
		...
		else if(Intersector.overlaps(touch, noB)){u.nextScreen=new GroupSettingsScreen(u,g);}
		return true;
	}

	private void toggleOff() {
		if(yes.isChecked()){yes.toggle();}
		if(no.isChecked()){no.toggle();}
