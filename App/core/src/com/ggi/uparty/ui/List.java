...
public class List {
	...
	public void give(ArrayList<Module> modules) {
		...
		for (int i = 0; i < this.modules.size(); i++) {
			...
			m.bounds = new Rectangle(0, bounds.y + bounds.height - ((modHeight + .015f) * (i + 1)) * u.h + scr, u.w,
					modHeight * u.h);
		}
	...
	public void move(float move) {
		...
		if (modules.size() * ((modHeight + .015f) * u.h) < bounds.height - (.02f * u.h) + scr) {
			scr = (modules.size() * ((modHeight + .015f) * u.h)) - (bounds.height - (.02f * u.h));
		}

		if (scr < 0) {
			scr = 0;
		}
		if (scr > (modules.size() - 1) * (modHeight + .015f) * u.h) {
			scr = (modules.size() - 1) * (modHeight + .015f) * u.h;
		}
				...
				if (bounds.height - modules.get(i).bounds.y <= (modHeight - .0475f) * u.h) {
					...
					if (fde > 0) {
						...
						if (modules.get(i).bounds.y >= bounds.y + bounds.height - ((modHeight + .015f)) * u.h) {
							modules.get(i).bounds = new Rectangle(0,
									bounds.y + bounds.height - ((modHeight + .015f)) * u.h, u.w, modHeight * u.h);
						}
