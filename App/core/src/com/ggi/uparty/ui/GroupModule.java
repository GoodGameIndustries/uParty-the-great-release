...
public class GroupModule extends Module {
	...
	public void draw(SpriteBatch pic, float fade) {
		...
		l.u.mediumFnt.draw(pic, group != null ? group.name : "Public", bounds.x + .05f * bounds.width,
	...
	public void touchDown(Rectangle touch) {
		if (group != null && Intersector.overlaps(touch, settingsB)) {
			settings.toggle();
		}
	}
	...
	public void touchUp(Rectangle touch) {
		if (group != null && Intersector.overlaps(touch, settingsB)) {
			l.u.nextScreen = new GroupSettingsScreen(l.u, group);
