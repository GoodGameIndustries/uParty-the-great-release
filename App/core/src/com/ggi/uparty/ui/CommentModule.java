...
public class CommentModule extends Module {
	...
	public void draw(SpriteBatch pic, float fade) {
		...
		a.setBounds(.05f * bounds.width + bounds.height, bounds.y + .1f * bounds.height,
				.9f * bounds.width - bounds.height, .8f * bounds.height);
		a.draw(pic, fade);
		...
		if (leftOverXP != 0 && lvAngle != 0) {
			lvBar.draw(pic, .05f * bounds.width, bounds.y + .05f * bounds.height, .9f * bounds.height,
					.9f * bounds.height);
		}

		l.u.supersmallFnt.setColor(1, 1, 1, fade);
		l.u.supersmallFnt.draw(pic, "" + lv, .05f * bounds.width + .9f * bounds.height / 2 - layout.width / 2,
