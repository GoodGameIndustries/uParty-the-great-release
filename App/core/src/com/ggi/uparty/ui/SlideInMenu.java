...
public class SlideInMenu {
	...
	public SlideInMenu(uParty u, Group g) {
		...
		newPost = makeMenuButton(
				new TextureRegionDrawable(new TextureRegion(u.assets.get("UI/Menu/Buttons/newUp.png", Texture.class))),
				new TextureRegionDrawable(
						new TextureRegion(u.assets.get("UI/Menu/Buttons/newDown.png", Texture.class))),
				"     New Post");
		groups = makeMenuButton(
				new TextureRegionDrawable(
						new TextureRegion(u.assets.get("UI/Menu/Buttons/groupUp.png", Texture.class))),
				new TextureRegionDrawable(
						new TextureRegion(u.assets.get("UI/Menu/Buttons/groupDown.png", Texture.class))),
				" Groups");
		newGroup = makeMenuButton(
				new TextureRegionDrawable(
						new TextureRegion(u.assets.get("UI/Menu/Buttons/newGroupUp.png", Texture.class))),
				new TextureRegionDrawable(
						new TextureRegion(u.assets.get("UI/Menu/Buttons/newGroupDown.png", Texture.class))),
				"        New Group");
		settings = makeMenuButton(
				new TextureRegionDrawable(
						new TextureRegion(u.assets.get("UI/Menu/Buttons/settingsUp.png", Texture.class))),
				new TextureRegionDrawable(
						new TextureRegion(u.assets.get("UI/Menu/Buttons/settingsDown.png", Texture.class))),
				"    Settings");
	...
	public void draw(SpriteBatch pic, float fade) {
		...
		newPost.setBounds(bounds.x + theta * .75f * u.w, bounds.y + .7975f * bounds.height, bounds.width,
				.0625f * bounds.height);
		groups.setBounds(bounds.x + theta * .75f * u.w, bounds.y + .735f * bounds.height, bounds.width,
				.0625f * bounds.height);
		newGroup.setBounds(bounds.x + theta * .75f * u.w, bounds.y + .6725f * bounds.height, bounds.width,
				.0625f * bounds.height);
		settings.setBounds(bounds.x + theta * .75f * u.w, bounds.y + .61f * bounds.height, bounds.width,
				.0625f * bounds.height);
		...
		pic.draw(u.assets.get("UI/Menu/Banner.png", Texture.class), bounds.x + theta * .75f * u.w,
				bounds.y + .86f * bounds.height, bounds.width, .14f * bounds.height);
		...
		u.largeFnt.draw(pic, "" + lv,
				bounds.x + theta * .75f * u.w + .05f * bounds.width + .12f * bounds.height / 2 - layout.width / 2,
				bounds.y + .87f * bounds.height + .12f * bounds.height / 2 + layout.height / 2);

		lvBar.setColor(new Color(1, 1, 1, fade));
		if (leftOverXP != 0 && lvAngle != 0) {
			lvBar.draw(pic, bounds.x + theta * .75f * u.w + .05f * bounds.width, bounds.y + .87f * bounds.height,
					.12f * bounds.height, .12f * bounds.height);
		}

		u.mediumFnt.setColor(247f / 255f, 148f / 255f, 29f / 255f, fade);

		layout.setText(u.mediumFnt, u.myAcc.u);
		u.mediumFnt.draw(pic, u.myAcc.u, bounds.x + theta * .75f * u.w + .16f * bounds.height,
				bounds.y + .93f * bounds.height + layout.height / 2);
	...
	public void tap(Rectangle touch) {
		...
		if (Intersector.overlaps(touch,
				new Rectangle(newPost.getX(), newPost.getY(), newPost.getWidth(), newPost.getHeight()))) {
			u.nextScreen = new CreateEventScreen(u, g);
		} else if (Intersector.overlaps(touch,
				new Rectangle(groups.getX(), groups.getY(), groups.getWidth(), groups.getHeight()))) {
			u.nextScreen = new GroupScreen(u);
		} else if (Intersector.overlaps(touch,
				new Rectangle(newGroup.getX(), newGroup.getY(), newGroup.getWidth(), newGroup.getHeight()))) {
			u.nextScreen = new NewGroupScreen(u);
		} else if (Intersector.overlaps(touch,
				new Rectangle(settings.getX(), settings.getY(), settings.getWidth(), settings.getHeight()))) {
