...
public class EventBar {
	...
	public void draw(SpriteBatch pic, float fade) {
		...
		if (s.scrolled > .378750f * u.h) {
			s.scrolled = .378750f * u.h;
		}
		...
		pic.draw(select == 0 ? descC : desc, bounds.x + bounds.width / 2 - .03f * u.w,
				bounds.y + (5f / 6f) * bounds.height - .03f * u.w, .06f * u.w, .06f * u.w);
		pic.draw(select == 1 ? locC : loc, bounds.x + bounds.width / 2 - .03f * u.w,
				bounds.y + (3f / 6f) * bounds.height - .03f * u.w, .06f * u.w, .06f * u.w);
		pic.draw(select == 2 ? timeC : time, bounds.x + bounds.width / 2 - .03f * u.w,
				bounds.y + (1f / 6f) * bounds.height - .03f * u.w, .06f * u.w, .06f * u.w);

		pic.draw(u.assets.get("UI/Filled.png", Texture.class), .10f * u.w,
				bounds.y + bounds.height - .189375f * u.h / 3f - s.scrolled / 3, .01f * u.w, .189375f * u.h / 3f);
	}

	public void touchUp(Rectangle touch) {
		if (Intersector.overlaps(touch, new Rectangle(bounds.x + bounds.width / 2 - .03f * u.w,
				bounds.y + (5f / 6f) * bounds.height - .03f * u.w, .06f * u.w, .06f * u.w))) {
			select = 0;
		} else if (Intersector.overlaps(touch, new Rectangle(bounds.x + bounds.width / 2 - .03f * u.w,
				bounds.y + (3f / 6f) * bounds.height - .03f * u.w, .06f * u.w, .06f * u.w))) {
			select = 1;
		} else if (Intersector.overlaps(touch, new Rectangle(bounds.x + bounds.width / 2 - .03f * u.w,
				bounds.y + (1f / 6f) * bounds.height - .03f * u.w, .06f * u.w, .06f * u.w))) {
