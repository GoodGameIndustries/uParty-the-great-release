...
public class PopUpMenu {
	...
	public PopUpMenu(MainScreen s) {
		...
		slideB = new Rectangle(bounds.width / 2 - .048f * s.u.w, .97f * bounds.height + bounds.y - .012f * s.u.h,
				.096f * s.u.w, .024f * s.u.h);
	...
	public void draw(SpriteBatch pic, float fade) {
		...
		if (open && slide != 0) {
			slide += (0 - slide) / 3;
		} else if (!open && slide != -.7525f * s.u.h) {
			slide += (-.7525f * s.u.h - slide) / 3;
		}
		float ratio = -(leftOverXP / neededToLvXP) * 360;
		if (slide > -.1f * s.u.h && lvAngle != ratio) {
		...
		if (open && slide > -.05f * s.u.h) {
			pic.draw(darken, 0, 0, s.u.w, s.u.h);
		}
		...
		s.u.largeFnt.draw(pic, s.u.myAcc.u, bounds.width / 2 - layout.width / 2,
				.93f * bounds.height + bounds.y - layout.height / 2);

		layout.setText(s.u.largeFnt, "" + lv);
		s.u.largeFnt.draw(pic, "" + lv, bounds.width / 2 - layout.width / 2,
				.75f * bounds.height + bounds.y - layout.height / 2);

		lvBar.setColor(new Color(1, 1, 1, fade));
		if (leftOverXP != 0 && lvAngle != 0) {
			lvBar.draw(pic, bounds.width / 2 - s.u.w / 8, .75f * bounds.height + bounds.y - layout.height - s.u.w / 8,
					s.u.w / 4, s.u.w / 4);
		}

		s.u.smallFnt.setColor(247f / 255f, 148f / 255f, 29f / 255f, fade);
		layout.setText(s.u.smallFnt, "(" + s.u.myAcc.e + ")");
		s.u.smallFnt.draw(pic, "(" + s.u.myAcc.e + ")", bounds.width / 2 - layout.width / 2,
				.87f * bounds.height + bounds.y - layout.height / 2);

		s.u.smallFnt.setColor(247f / 255f, 148f / 255f, 29f / 255f, fade);
		layout.setText(s.u.smallFnt, s.u.controller.getLat() + ":" + s.u.controller.getLong());
		// s.u.smallFnt.draw(pic,
		// s.u.controller.getLat()+":"+s.u.controller.getLong(),
		// bounds.width/2-layout.width/2,
		// .87f*bounds.height+bounds.y-layout.height/2);

		layout.setText(s.u.smallFnt, "Groups:");
		s.u.smallFnt.draw(pic, "Groups:", bounds.width / 2 - layout.width / 2,
				.34f * bounds.height + bounds.y - layout.height / 2);
	...
	public void touchDown(Rectangle touch) {
		if (Intersector.overlaps(touch, postB)) {
			post.toggle();
		} else if (Intersector.overlaps(touch, settingsB)) {
			settings.toggle();
		} else if (Intersector.overlaps(touch, groupB)) {
			group.toggle();
		} else if (Intersector.overlaps(touch, l.bounds)) {
			l.touchDown(touch);
			initTouch = true;
		}
	}
	...
	public void touchUp(Rectangle touch) {
		...
		if (!initTouch) {
			if (Intersector.overlaps(touch, postB)) {
				s.u.nextScreen = new CreateEventScreen(s.u, s.g);
			}
			// else if(Intersector.overlaps(touch,
			// settingsB)){s.u.myAcc=null;s.u.settings=true;s.u.setScreen(new
			// LoadScreen(s.u));}
			else if (Intersector.overlaps(touch, settingsB)) {
				s.u.setScreen(new SettingsScreen(s.u));
			} else if (Intersector.overlaps(touch, groupB)) {
				s.u.nextScreen = new NewGroupScreen(s.u);
			} else if (!open) {
				open = !open;
			}
		}
		if (Intersector.overlaps(touch, l.bounds)) {
			l.touchUp(touch);
		}
		initTouch = false;
