...
public class MemberModule extends Module {
	...
	public void draw(SpriteBatch pic, float fade) {
		...
		l.u.smallFnt.draw(pic, m.u + " (" + m.e + ")", bounds.x + .05f * bounds.width,
				bounds.y + bounds.height / 2 + layout.height / 2);

		layout.setText(l.u.smallFnt, m.rank != 2 ? "Member" : "Owner");
		l.u.smallFnt.draw(pic, m.rank != 2 ? "Member" : "Owner", bounds.x + .95f * bounds.width - layout.width,
				bounds.y + bounds.height / 2 + layout.height / 2);

		MemberModule mod = (MemberModule) l.modules.get(0);
		if (mod.m.e.equals(l.u.myAcc.e) && m.rank != 2) {
			remove.draw(pic, fade);
			isOwner = true;
		}
	}
