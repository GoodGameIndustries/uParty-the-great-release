...
public class RadialSprite implements Drawable {
	...
	private final void vert(final float[] verts, final int offset, final float x, final float y, final float u,
			final float v) {
	...
	protected void calculate(float x, float y, float width, float height, float angle, float u0, float v0, float u1,
			float v1) {
		if (!this.dirty && this.x == x && this.y == y && this.angle == angle && this.width == width
				&& this.height == height && this.u1 == u0 && this.v2 == v1 && this.u2 == u1 && this.v2 == v1)
			return;
		this.x = x;
		this.y = y;
		this.width = width;
		...
		final float ax = MathUtils.cosDeg(angle + angleOffset); // positive
																// right,
																// negative left
		final float ay = MathUtils.sinDeg(angle + angleOffset); // positive top,
																// negative
																// bottom
	...
	public void draw(final SpriteBatch batch, final float x, final float y, float width, float height,
			final float angle) {
		if (width < 0) {
			scaleX = -1f;
			width = -width;
