...
public class RefreshModule {
	...
	public void draw(SpriteBatch pic, float fade) {
		...
		pic.draw(icon, bounds.width / 2 - bounds.height / 4, bounds.y + bounds.height / 4, bounds.height / 2,
				bounds.height / 2);
		if (s.events.refresh) {
			pic.draw(load, bounds.width / 2 - bounds.height / 4, bounds.y + bounds.height / 4, bounds.height / 4,
